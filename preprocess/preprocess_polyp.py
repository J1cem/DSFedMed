...
data_path = '/mnt/diskB/zhw/fundus'
target_path = '/mnt/diskB/zhw/fundus_1024_new'
if not os.path.exists(target_path):
    os.makedirs(target_path)
client_data_list = []
slice_num =[]
for client_idx in range(client_num):
    print('{}/test/real_image/{}/*'.format(data_path,client_name[client_idx]))
    client_data_list.append(glob('{}/test/real_image/{}/*'.format(data_path,client_name[client_idx])))
...
for client_idx in range(client_num):
    ...
    for fid, filename in enumerate(client_data_list[client_idx]):
        ...
        labelname = filename.replace('image', 'label', 1)
        # print(labelname)
        if os.path.exists(labelname):
            label_data = np.array(Image.open(labelname)).astype(np.uint8)
            ...
            unique_vals_raw, counts_raw = np.unique(label, return_counts=True)
            for val, count in zip(unique_vals_raw, counts_raw):
                raw_pixel_counts[val] += count
            # print("所有像素值:", np.unique(label))
            if label.max() <= 1:
                # already 0/1
                label = label
            else:

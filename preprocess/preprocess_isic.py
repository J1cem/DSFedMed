...
data_path = '/mnt/diskC/zhw/my_nnunet/train_kva_syn'
# data_path = '/mnt/diskC/zhw/my_nnunet/fundus_syn_all'
target_path = '/mnt/diskB/zhw/polyp_1024_new'
if not os.path.exists(target_path):
    os.makedirs(target_path)
client_data_list = []
slice_num =[]
for client_idx in range(client_num):
    print('{}/images/*'.format(data_path))
    client_data_list.append(glob('{}/images/*'.format(data_path)))
...
for client_idx in range(client_num):
    ...
    for fid, filename in enumerate(client_data_list[client_idx]):
        ...
        labelname = filename.replace('images','labels').replace('img','mask')

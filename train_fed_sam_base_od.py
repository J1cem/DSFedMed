...
import cfg
args = cfg.parse_args()
snapshot_path = "../fed-sam-output-od/" + args.exp + "/"
...
elif args.data=='Nuclei_3':
    client_name = ['MoNuSAC2020','TNBC','MoNuSAC2018']
    data_path = '/mnt/.../lyx/Nuclei_1024'
...
# when removing unseen site, ignore the red lines and the following line
...
def validation(test_net):
    ...
    if args.num_classes==2:
        dice_avg, eiou_avg, dice_avg_cup, eiou_avg_cup = val(-1,test_net)
...
def test(site_index, test_net):
    ...
    inference_times = []  # 用来记录每张图推理时间

    for fid, filename in enumerate(test_data_list):
        data = np.load(filename) / 255.0
        mask_data = np.load(filename.replace("data", "label", 1))
        image = np.expand_dims(data[..., :3].transpose(2, 0, 1), axis=0)
        if mask_data.ndim == 2:
            mask_data = np.expand_dims(mask_data, axis=-1)
        mask = np.expand_dims(mask_data.transpose(2, 0, 1), axis=0)
        ...
        with torch.no_grad():
            ...
            pred, _, _ = test_net.mask_decoder(
                image_embeddings=image_encoded,
                image_pe=test_net.prompt_encoder.get_dense_pe(),
                sparse_prompt_embeddings=se,
                dense_prompt_embeddings=de,
                multimask_output=False,
            )
            ...
            inference_times.append(end_time - start_time)  # 记录本张图推理时间
    ...
    logging.info(f"Average inference time per image: {avg_inference_time:.4f} seconds")
...
if __name__ == "__main__":
    ...
    # validation(net_clients[1], client_weight)
    ...
    for epoch_num in tqdm(range(start_epoch,max_epoch), ncols=70):
        ...
        for client_idx in source_site_idx:
            ...
            for i_batch, sampled_batch in enumerate(dataloader_current):
                ...
                # calculate
                '''
                total_params = sum(p.numel() for p in net_current.parameters())
                trainable_params = sum(p.numel() for p in net_current.parameters() if p.requires_grad)
                ...
                if iter_num % 10000==0:
                    ...
                    # break
        ## model aggregation
        update_global_model_all(net_clients, client_weight)
        validation(net_clients[0])
        ...
        if epoch_num % 5==0 or epoch_num==max_epoch-1:
            ...
            with open(os.path.join(snapshot_path, 'evaluation_result.txt'), 'a') as f:
                ...
                if args.num_classes==2:
                    ...
                    # the following lines remove unseen site setting
            ...
            ## save model
            if epoch_num % 25==0 or epoch_num==max_epoch-1:

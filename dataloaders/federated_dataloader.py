...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if image_patch.ndim == 2 or image_patch.shape[-1] == 1:  # Grayscale image (H, W) or (H, W, 1)
            image_patch = np.repeat(image_patch, 3, axis=-1)
        ...
        if mask_patch.ndim == 2:  # (H, W)
            mask_patch = np.expand_dims(mask_patch, axis=-1)  # 添加通道维度 (H, W, 1)
        
        if mask_patch.shape[-1] == 3:  # 三通道转单通道
            mask_patch = np.mean(mask_patch, axis=2, keepdims=True)
...
class MultiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # 确定从哪个数据集获取数据
        dataset_idx = 0
        while idx >= self.dataset_sizes[dataset_idx]:
            idx -= self.dataset_sizes[dataset_idx]
            dataset_idx += 1
        # 获取该数据集中的数据
        sample = self.datasets[dataset_idx][idx]
        # 在 sample 字典中添加 dataset_idx
